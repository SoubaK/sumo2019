...
AsservissementMascart::AsservissementMascart(int dir, int pwm, int brake, int pinA, int pinB) {
  targetSpeed = 0.3;
  m_dir = dir;
  m_pwm = pwm;
  m_brake = brake;
...
void AsservissementMascart::init() {
  pinMode(m_dir, OUTPUT);
  pinMode(m_pwm, OUTPUT);
  pinMode(m_brake, OUTPUT);
  //attachInterrupt(m_pinA, coder, CHANGE);
  for(int i=0;i<=4;i++){
        umot_precedentes[i] = 0;
        theta_precedents[i] = 0;
    }
}

void AsservissementMascart::run(double theta_increment) {
  ...
  cmd = constrain(umot_current, -255, 255);
    ...
    for(int i=0;i<4;i++){
        ...
        theta_precedents[4-i] = theta_precedents[4-i-1];
    }
    theta_precedents[0] = theta_current;
...
/*
static void AsservissementMascart::coder() {
